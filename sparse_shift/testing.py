...
def test_pooled_mechanism(Xs, m, parents, test='kci', test_kwargs={}):
    ...
    if test == 'fisherz':
        ...
        condition_set = tuple(np.where(parents > 0)[0] + 1)
        # 原本kci为str，需要利用CIT将其变成kci类
        kci_class = CIT(data, test)
        pvalue = kci_class(data, 0, m + 1, condition_set)
    else:
        ...
        for e2 in range(e1 + 1, E):
            if sum(parents) == 0:
                ...
                if test == 'kcd':
                    ...
                    condition_set = tuple(np.where(parents > 0)[0] + 1)
                    pvalue = fisherz(0, m+1, condition_set)
                elif test == 'kci':
                    assert len(Xs) > 1
                    # Test X \indep E | PA_X
                    data = np.block([
                    ...
                    # 原本kci为str，需要利用CIT将其变成kci类
                    kci_class = CIT(data, test)
                    pvalue = kci_class(0, m + 1, condition_set)
